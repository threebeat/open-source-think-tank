...
**Status:** Work Package **4.5A.1** active (integrity closure). Phase 4.1–**4.3** owner-approved complete. Phase **4.4** merged (PR #20). Phase **4.5A** merged (PR #21) with **conditional acceptance**. Builds on [architecture-phase-3.md](./architecture-phase-3.md) and Phase 2 dual-mode isolation.  
...
This document describes the Phase 4 product/architecture contract. **Public Input remains fail-closed for live Pol.is.** Package **4.5A.1** closes database-boundary immutability (report-root transition matrix; child INSERT/UPDATE/DELETE with parent `FOR UPDATE`), persists suppression-policy provenance at publish, separates data provenance from actor synthetic flags, invalidates legacy estimated counts (`requires_reimport`), and adds PostgreSQL concurrency proofs. **4.5B+ public IA rebuild must not merge until 4.5A.1 exits.**
