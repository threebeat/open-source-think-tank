...
| Gated public projection DTO (3.6/3.10) | Anonymous visitor allowlist | Public only when published; empty shell when published but no eligible claim/evidence | Derived read model | No account/reviewer IDs, private notes, private disclosure, raw audit; evidence public conflict summaries included; quality pending/rejected excluded |
...
| Public-demo discovery query state | Shareable advanced search filters | Local URL only; no accounts | Ephemeral browser | Never used for gated draft search (3.11 uses separate ACL search) |
| Gated workspace search result DTO (3.11/3.12) | Authorized metadata hits; SQL-bounded count+page | Session + `workspace.search`; audience-scoped | Ephemeral response | Entity type, safe title, topic label/slug where permitted, workflow/quality/visibility labels, updatedAt, authorized href only — no account IDs, contacts, private notes, private disclosure, invites, verification, raw audit, revision snapshots, or internal admission class |
| Account export workspace section (3.11/3.12) | Own Phase 3 claims/evidence/links/disclosures/owner revision history + minimal topic context | Account holder only | On download | Structured ownership checks + serialized foreign-account abort; thrown failures → sanitized `ACCOUNT_EXPORT_UNAVAILABLE`; no other accounts’ content or staff private notes |
| Staff topic export package (3.11/3.12) | Selected topic package for staff work | Reviewer or administrator (`topics.export_staff`) | On download | Allowlisted projector; revalidated source URLs (never fetched); audit counts only; thrown failures → sanitized `STAFF_EXPORT_UNAVAILABLE` |
| Alpha reset receipt / `alpha.reset_executed` (3.12 / closure) | New audit-chain root after wipe (not continuity with erased pre-reset ledger) | Operator CLI only (`OPERATOR_RESET_SECRET`) | Survives as the new audit chain head after wipe | Metadata only: timestamp, operator label, reason, DB fingerprint, schema/migration version, commit SHA, coarse family counts, manifest version/hash, explicit `receiptProvenance` (`operational` \| `synthetic_smoke`) — never PII, bodies, tokens, or row snapshots |
| Operational assent document definitions (`document_versions`, regenerated) | Required published documents so post-reset bootstrap/onboarding can proceed | Staff/account assent presentation surfaces | Regenerated from checked-in non-participant catalog on alpha reset; not participant data | Provisional alpha placeholders — not counsel-approved legal language; participant assent records/presentations remain wipeable |
