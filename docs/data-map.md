...
| Gated workspace search result DTO (3.11) | Authorized metadata hits | Session + `workspace.search`; audience-scoped | Ephemeral response | Entity type, safe title, topic label/slug where permitted, workflow/quality/visibility labels, updatedAt, authorized href only — no account IDs, contacts, private notes, private disclosure, invites, verification, raw audit, revision snapshots |
| Account export workspace section (3.11) | Own Phase 3 claims/evidence/links/disclosures/owner revision history + minimal topic context | Account holder only | On download | Structured ownership checks + serialized foreign-account abort; no other accounts’ content or staff private notes |
| Staff topic export package (3.11) | Selected topic package for staff work | Reviewer or administrator (`topics.export_staff`) | On download | Allowlisted projector; revalidated source URLs (never fetched); audit counts only |
