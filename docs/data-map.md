...
| Alpha reset receipt / `alpha.reset_executed` (3.12) | Operator wipe continuity record | Operator CLI only (`OPERATOR_RESET_SECRET`) | Survives as the new audit chain head after wipe | Metadata only: timestamp, operator label, reason, DB fingerprint, schema/migration version, commit SHA, coarse family counts, manifest version/hash — never PII, bodies, tokens, or row snapshots |
