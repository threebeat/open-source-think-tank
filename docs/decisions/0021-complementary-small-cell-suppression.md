...
3. Synthetic public-demo keeps the provisional threshold of **5** for engineering and education. That number is **not** a production privacy decision.
4. Production threshold remains open: privacy review (**OQ27**) plus explicit owner approval of the numeric production threshold (**OQ35**) before gated published reports use any non-provisional value.
5. Suppression runs at **projection time** for public DTOs (and must be re-checked on publish). Stored import rows may retain pre-suppression aggregate inputs as protected/staff data under gated access — never as public DTO fields.
6. Differencing across **report versions** must not re-expose a previously suppressed cell: publication/review UX and public history views apply suppression consistently (including complementary rules) to each version’s public projection.
7. This ADR does **not** settle the production threshold, authorize live Pol.is, or weaken forbidden-key / aggregate-only rules.
