...
26. **Pol.is vendor / data-processing approval** — Unresolved / **blocker for LIVE activation** (not for 4.3–4.4 engineering). Phase 4.2 recorded a sourced assessment ([public-input-provider-assessment.md](./public-input-provider-assessment.md)) with verdict **insufficient information**. Phase 4.3 landed the institutional lifecycle and a fail-closed embed shell; Phase 4.4 adds aggregate-only report ingest + moderation engineering — **neither package authorizes live install**. Live remains blocked until hosted vs self-hosted selection, written DPA, subprocessors (incl. LLM report processors), residency, retention/deletion, breach/SLA, export schema, permitted-services register addendum, and owner `ENABLE LIVE POLIS…` authorization clear ([phase-4-plan.md](./phase-4-plan.md) §11d–§11e, [ADR 0012](./decisions/0012-public-input-provider-boundary.md), activation checklist in `src/lib/public-input/lifecycle/activation.ts`).

27. **Production small-cell suppression threshold** — Unresolved. Synthetic demo uses a provisional threshold of **5**; production threshold requires privacy review. Phase 4.2 requires suppressed cells to render as explicit “Suppressed” (share `null`), never as `0%`. Phase **4.4** lands **complementary** small-cell suppression engineering ([ADR 0021](./decisions/0021-complementary-small-cell-suppression.md)); Phase **4.5A** requires **exact integer `participantCount`** inputs so cells cannot round upward across the threshold — that still does **not** settle the production numeric threshold (see also OQ35 / OQ36).
...
35. **Production small-cell threshold owner approval** — Unresolved. After privacy review (OQ27), the numeric production threshold used by gated published Public Input reports still needs **explicit owner approval** before replacing the demo provisional value of **5**. Complementary suppression mechanics (4.4 / ADR 0021) and exact-count inputs (4.5A) may ship while this gate remains open; they must not silently adopt a production number.

36. **Exact-count opinion-group privacy rule** — Unresolved / needs privacy re-review before production-capable claims. Phase **4.5A** engineering adopts exact integer `participantCount` per opinion group, partition-sum validation against `participationCount`, rejection of duplicate normalized labels, and suppression that never rounds a sub-threshold cell upward ([ADR 0018](./decisions/0018-aggregate-only-canonical-import-format.md) `@1.1`, [ADR 0021](./decisions/0021-complementary-small-cell-suppression.md)). Owner/privacy must still confirm this exact-count rule (versus any documented conservative alternative) as the production privacy contract, independent of the numeric threshold in OQ27/OQ35.
