...
export async function POST(request: Request, context: RouteContext) {
  ...
  const { getGatedDb } = await import("@/lib/auth/runtime");
  const { moderateEvidence } = await import("@/lib/moderation/service");
  const result = await moderateEvidence(getGatedDb(), {
    actorAccountId: gated.session.accountId,
    evidenceSubmissionId: id,
  ...
  return NextResponse.json(
    {
      evidenceSubmissionId: result.value.evidence.id,
      moderationVisibility: result.value.evidence.moderationVisibility,
      updatedAt: result.value.evidence.updatedAt.toISOString(),
