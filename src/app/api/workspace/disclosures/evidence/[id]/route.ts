...
export async function GET(_request: Request, context: RouteContext) {
  ...
  const { getOwnEvidenceDisclosure } = await import("@/lib/conflicts/disclose");
  const result = await getOwnEvidenceDisclosure(getGatedDb(), {
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  const gatedBody = await gateAuthenticatedMutation({
    ...
    family: "edit_resubmit_withdraw_disclosure",
  });
  if (!gatedBody.ok) {
