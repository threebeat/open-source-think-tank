...
/** POST — publish a reviewed report (conversation must be closed). */
export async function POST(request: Request, context: RouteContext) {
  ...
  const { publishReport } = await import("@/lib/public-input/reports/service");
  const result = await publishReport(getGatedDb(), {
