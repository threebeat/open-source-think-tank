...
export async function POST(request: Request) {
  ...
  const { revokePlatformRole } = await import("@/lib/authz/role-changes");
  const result = await revokePlatformRole(getGatedDb(), {
  ...
  if (!result.ok) {
    const status =
      ...
      result.code === "AUTHZ_LAST_ADMIN_FORBIDDEN" ||
      result.code === "AUTHZ_LAST_AUDITOR_FORBIDDEN" ||
