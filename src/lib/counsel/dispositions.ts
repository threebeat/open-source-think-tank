...
export const COUNSEL_DISPOSITIONS: Record<string, CounselDisposition> = {
  statutory_membership: {
    ...
    status: "blocking",
    scope: "Full gate; no product claim of statutory membership",
    recordedDate: "2026-08-09",
    recordedBy: "Phase 2.1 contract author",
    counselSource: "none — still blocking",
    projectOwnerApproval: "n/a",
    affectedPackages: ["2.1", "2.4", "2.5", "2.8"],
  },
  electronic_assent: {
    id: "electronic_assent",
    status: "blocking",
    scope:
      "Full gate; no “not legally reviewed” doc may become active assent",
    recordedDate: "2026-08-09",
    recordedBy: "Phase 2.1 contract author",
    counselSource: "none — still blocking",
    projectOwnerApproval: "n/a",
    affectedPackages: ["2.6", "2.8"],
  },
  eligibility_geography: {
    ...
    counselSource: "none — still blocking",
    projectOwnerApproval: "n/a",
    affectedPackages: ["2.7", "2.8"],
  },
  political_opinion_verification: {
    id: "political_opinion_verification",
    status: "blocking",
    scope:
      "Full gate; keep identity store separated from opinion/pseudonym maps",
    recordedDate: "2026-08-09",
    recordedBy: "Phase 2.1 contract author",
    counselSource: "none — still blocking",
    projectOwnerApproval: "n/a",
    affectedPackages: ["2.7", "2.9", "2.10", "2.11"],
  },
...
export function activationCounselAllowsRealAccounts(): boolean {
  return ACTIVATION_COUNSEL_GATE_IDS.every((id) => {
    const row = COUNSEL_DISPOSITIONS[id];
    return (
      row?.status === "cleared" || row?.status === "conditionally_cleared"
    );
  });
}
